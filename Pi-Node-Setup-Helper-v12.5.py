...
# -*- coding: utf-8 -*-
"""
Pi Node Setup Helper - Web Application v12.5
Supports Windows
...
# Setting up advanced firewall rules (1:1 from reference script)
...
iptables -t nat -A PREROUTING -i "$DEFAULT_INTERFACE" -p tcp -m multiport --dport $FW_TCP -j DNAT --to-destination $CLIENT_VPN_IP
iptables -t nat -A PREROUTING -i "$DEFAULT_INTERFACE" -p udp -m multiport --dport $FW_UDP -j DNAT --to-destination $CLIENT_VPN_IP
...
def get_status():
    ...
    for name in TASKS_ORDER:
        ...
        if name == "wsl_setup" and IS_WINDOWS:
             # For WSL, installer_ready is just for the kernel update part
             pass
...
def open_directory():
    ...
    try:
        ...
        if d and os.path.isdir(d):
            if IS_WINDOWS:
                # Using shell=True with explorer ensures it gets brought to foreground more reliably
...
def check_for_updates():
    try:
        ...
        if r.status_code == 200:
            ...
            # Check if assets contain an installer (exe or dmg)
            ...
            print(f"[*] Update check: Current={current_tuple}, Latest={latest_tuple} (Tag: {latest_tag})")
    ...
    try:
        ...
        def worker():
            ...
            try:
                ...
                report_telemetry_error({"type": "worker_error", "task": task_name, "action": action_type, "error": str(e)})
            finally:
                with operation_lock:
                    if op_id in operation_logs:
        ...
        report_telemetry_error({"type": "internal_server_error", "task": task_name, "error": str(e)})
...
def get_qr_code():
    try:
        # Check if qr_code.txt exists in the same folder as the EXE first (user override)
        local_path = os.path.join(os.path.dirname(sys.executable), "qr_code.txt")
        if not os.path.exists(local_path):
            # Fallback to current working dir (development) or bundled one
...
def shutdown_app():
    print("[*] Shutdown requested by frontend. Goodbye!")
    # Non-blocking delay to allow the response to reach the frontend
    threading.Thread(target=lambda: (time.sleep(1), os._exit(0))).start()
    return jsonify({"ok": True})

def shutdown_checker():
    # Only active when not debugging (if needed)
    while True:
        time.sleep(5)
        # If no heartbeat for 15 seconds, exit
        if time.time() - last_heartbeat > 15:
            print("[*] Browser closed or heartbeat lost. Exiting app...")
            os._exit(0)
...
if __name__ == '__main__':
    ...
    # Initial telemetry from backend
    ...
    # Run the webapp for productive Enterprise use using Waitress WSGI server
    try:
